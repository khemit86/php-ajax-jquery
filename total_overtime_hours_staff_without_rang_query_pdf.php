...
##### get all shift count of staff start ######
	...
	$get_total_staff_hours_rows = 0;	
		...
		$get_total_staff_hours_rows = 0;
		$get_total_staff_hours_query = "SELECT sum(time_to_sec(TIMEDIFF(CONCAT_WS(' ', rota.rota_period,rota.shift_end_time ),CONCAT_WS(' ', rota.rota_period,rota.shift_start_time )))) as total_staff_seconds, employee.employee_number,employee.first_name,employee.last_name,employee.job_title,employee.weekly_contract_hours_limit,department.department_name FROM employee LEFT JOIN rota ON employee.employee_number = rota.employee_number LEFT JOIN department ON  employee.department_id = department.department_id WHERE rota.department_id = '".$_SESSION['login_department_id']."' GROUP BY employee.employee_number ORDER BY total_staff_seconds DESC";
	...
	$pdf_html = '';
	$pdf_html  .= '<h1 style="text-align:center">Total Overtime Hours Of Staff '.$_SESSION['login_department_name'].'</h1><table border="1" width="100%" cellpadding="20" cellspacing="0">
	<thead>
		<tr>
	...
	{		
		while ($get_total_staff_hours_obj = $get_total_staff_hours_result->fetch_object()) {
			...
			if($get_total_staff_hours_obj->total_staff_seconds > ($get_total_staff_hours_obj->weekly_contract_hours_limit * 3600)){	
				...
				$pdf_html  .=  '<td>'. seconds_to_hoursminutes($over_time_amount) .'</td>';
				
			}else{
				$pdf_html  .=  '<td> 00:00 </td>';
			}							
