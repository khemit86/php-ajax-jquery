...
if(!empty($_POST))
{
   $week = $_POST['week'];
    ...
    $table_day_header = get_day_of_month($week,$numeric_month_array[$month],$year);
    ...
    if ($staff_rows > 0) {
		while($staff_obj = $staff_result->fetch_object()) {
			...
			$week_dynamic_date = $year."-".$numeric_month_array[$month]."-01";
			$get_shift_per_week_result = $con->query("SELECT rota_id FROM rota WHERE employee_number = '".$staff_obj->employee_number ."' AND rota.department_id = '".$_SESSION['login_department_id']."' AND WEEK (rota_period,1) = WEEK('".$week_dynamic_date."',1) + ".$week." AND YEAR( rota_period) = YEAR( '".$week_dynamic_date."' ) AND MONTH(rota_period) = '".$numeric_month_array[$month]."' group by rota_period");
			...
			$get_days = get_days($week,$numeric_month_array[$month],$year);
			...
			$month_first_date = $year."-".$numeric_month_array[$month]."-01";
			$get_count_query = "SELECT rota.* FROM rota ";
			$get_count_query .= "WHERE employee_number = '".$staff_obj->employee_number."' ";
			$get_count_query .= " AND WEEK (rota_period,1) = WEEK('".$month_first_date."',1) + '".$week."' AND YEAR( rota_period) = YEAR( '".$month_first_date."' ) AND MONTH(rota_period) = '".$numeric_month_array[$month]."' AND rota.department_id = '".$_SESSION['login_department_id']."' ";
    ...
    echo json_encode(array("success"=>"1","rota_html"=>$rota_html,"login_department_name"=>$_SESSION['login_department_name']));exit;    
}    
?>