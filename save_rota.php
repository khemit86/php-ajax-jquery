...
if (!empty($_POST['rota'])) {
	...
	$day	=	$_POST['rota']['shift_common_data']['day'];
	$date	=	$_POST['rota']['shift_common_data']['date'];
	...
	{
		echo json_encode(array("success"=>"0","validation_key"=>implode(",",$validation_count_row_unique),"validation_message"=>implode(",",$validation_message)));exit;
	} */
	//else
	//{	
		/* foreach($_POST['rota']['shift_type_data'] as $key=>$value){
		...
		//{	
			foreach($_POST['rota']['shift_type_data'] as $key=>$value){
				...
				if (isset($value['rota_id']) && !empty($value['rota_id'])) {
					if (!empty($value['customer_id']) && !empty($value['shift_type_id']) && !empty($value['shift_start_time']) && !empty($value['shift_end_time'])) { 
						$rota_update_query = "UPDATE rota SET customer_id='".$value['customer_id']."',shift_type_id='".$value['shift_type_id']."',allocation_id='".$allocation_id."',shift_start_time='".$value['shift_start_time']."',shift_end_time='".$value['shift_end_time']."',shift_details='".$shift_details."',shift_break='".$shift_break."',travel_start_time='".$travel_start_time."',travel_end_time='".$travel_end_time."' WHERE rota_id='".$value['rota_id']."' ";
						$con->query($rota_update_query);
					} if (empty($value['customer_id']) && empty($value['shift_type_id'])) {
						$delete_rota_query = "DELETE FROM rota WHERE rota_id = '".$value['rota_id']."' ";
