...
##### get all shift count of staff start ######
	...
	if($get_cancelled_staff_count_rows > 0)
	{
		while ($get_total_shift_staff_obj = $get_cancelled_staff_count_result->fetch_object()) {
			if(isset($get_total_shift_staff_obj->first_name) && !empty($get_total_shift_staff_obj->first_name) && isset($get_total_shift_staff_obj->last_name) && !empty($get_total_shift_staff_obj->last_name)){
				$user_name =  ucwords($get_total_shift_staff_obj->first_name.' '.$get_total_shift_staff_obj->last_name);
			}else{
				$user_name =  " ";			
			}
			...
			$pdf_html  .=  '<td>'.$user_name .'</td>';
			$pdf_html  .=  '<td>'.$get_total_shift_staff_obj->job_title .'</td>';
			$pdf_html  .=  '<td>'.$get_total_shift_staff_obj->department_name .'</td>';
			$pdf_html  .=  '<td>'.$shift_cancellation_reason[$get_total_shift_staff_obj->cancellation_reason] .'</td>';
			$pdf_html  .=  '<td>'.$get_total_shift_staff_obj->cancellation_details .'</td>';
			$pdf_html  .=  '<td>'.date("l jS, F Y",strtotime($get_total_shift_staff_obj->cancellation_date)) .'</td>';
			$pdf_html  .= '</tr>';
		}
	}
	else
