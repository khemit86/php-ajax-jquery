...
$date = $_POST['date'];
$delete_all_cancel_reason = $_POST['delete_all_cancel_reason'];
$delete_all_cancel_description = $_POST['delete_all_cancel_description'];
$rota_period = $year."-".$month."-".$date;
/*     $delete_rota_query = "DELETE FROM rota ";
    $delete_rota_query .= "WHERE ";
if(!empty($emp_number))
{
    $delete_rota_query .= "employee_number = '".$emp_number."' AND department_id = '".$_SESSION['login_department_id']."' ";
}
if(!empty($year) && !empty($month) && !empty($date))
{    
    $delete_rota_query .= " AND rota_period = '".$rota_period."' ";
}
$con->query($delete_rota_query); */


	$get_delete_result = $con->query("SELECT * FROM rota WHERE employee_number = '".$emp_number."' AND department_id = '".$_SESSION['login_department_id']."' AND rota_period = '".$rota_period."' ");
	$get_delete_result_rows = mysqli_num_rows($get_delete_result);
	if($get_delete_result_rows > 0)
	{	
		
		while($get_delete_result_obj = $get_delete_result->fetch_object())
		{
			/* $con->query("INSERT INTO shift_cancellation (department_id,employee_number,cancelled_shift_type_id,rota_period,cancellation_reason,cancellation_details,cancellation_date) VALUES ('".$get_delete_result_obj->department_id ."','".$get_delete_result_obj->employee_number ."','".$get_delete_result_obj->shift_type_id."','".$get_delete_result_obj->rota_period."','".$delete_all_cancel_reason."','".$delete_all_cancel_description."','".date('Y-m-d')."') "); */
			
			
			$con->query("INSERT INTO shift_cancellation (rota_period,employee_number,customer_id,cancelled_shift_type_id,allocation_id,department_id,shift_start_time, 	shift_end_time,shift_details,shift_break,travel_start_time,travel_end_time,cancellation_reason,cancellation_details,cancellation_date) VALUES ('".$get_delete_result_obj->rota_period ."','".$get_delete_result_obj->employee_number ."','".$get_delete_result_obj->customer_id ."','".$get_delete_result_obj->shift_type_id."','".$get_delete_result_obj->allocation_id."','".$get_delete_result_obj->department_id."','".$get_delete_result_obj->shift_start_time."','".$get_delete_result_obj->shift_end_time."','".$get_delete_result_obj->shift_details."','".$get_delete_result_obj->shift_break."','".$get_delete_result_obj->travel_start_time."','".$get_delete_result_obj->travel_end_time."','".$delete_all_cancel_reason."','".$delete_all_cancel_description."','".date('Y-m-d')."') ");
			
			
			$rota_id = $get_delete_result_obj->rota_id;
    ...
    $get_count_query .= "WHERE employee_number = '".$staff_obj->emp_number."' ";
    $get_count_query .= " AND WEEK (rota_period,1) = WEEK('".$month_first_date."',1) + '".$week_no."' AND YEAR( rota_period) = YEAR( '".$month_first_date."' ) AND MONTH(rota_period) = '".$numeric_month_array[$month]."' AND rota.department_id = '".$_SESSION['login_department_id']."' ";
    ...
    {    
        while($get_rota_count_obj = $get_count_rota_result->fetch_object()) {
           ...
           elseif($total_hours > (number_format($staff_obj->weekly_contract_hours_limit)*3600))
            {
