...
##### get all shift houres for date range of staff start ######
	...
	if(!empty($date_from) &&  !empty($date_to) )
	{
		...
		$get_daily_shift_type_query = "SELECT rota.rota_period,employee.employee_number,employee.first_name,employee.last_name,employee.job_title,department.department_name,shift_type.shift_type FROM rota LEFT JOIN employee ON rota.employee_number = employee.employee_number LEFT JOIN shift_type ON rota.shift_type_id = shift_type.shift_type_id LEFT JOIN department ON rota.department_id = department.department_id WHERE DAYNAME(`rota_period`) IN ('Saturday','Sunday') AND rota.rota_period >= '" .$date_from."' AND rota.rota_period <= '".$date_to."' AND rota.department_id = '".$_SESSION['login_department_id']."' ";
...
<link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
	...
	<div class="all-department-employee-table">	
		<form action="list_of_weekend_shift_staff_range_query.php" method="POST" id="list_of_weekend_shift_staff_range_query_form">
		...
		<ul>
			...
			<li><a href="javascript:;" id="list_of_weekend_shift_rang_pdf_btn" target="_blank" class="btn btn-picton-blue shift_month_report"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Download</a></li>
    ...
    <script type="text/javascript">
       ...
       $(document).ready(function(){
			...
			$("#list_of_weekend_shift_staff_range_query_form").submit(function(){
				if(check_validation()){
					$("#list_of_weekend_shift_staff_range_query_form").submit();
				}
				return false;
			});
			 $("#list_of_weekend_shift_rang_pdf_btn" ).click(function() {
				if(check_validation()){
					var date_from = $("#date_from").val();
					var date_to = $("#date_to").val();
					window.open(SITE_URL+'list_of_weekend_shift_staff_range_query_pdf.php?date_from='+ date_from +'&date_to='+ date_to);
				}
				return false;
			}); 
